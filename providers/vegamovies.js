...
function makeStream(_, title, url, quality, headers, mediaInfo, fallbackQ = 'HD') {
    ...
    const sortTag = getSortTag(Q_WEIGHTS[nq] || 0);
    const shortLabel = `${PROVIDER} • ${nq.toUpperCase()}${imax}${host ? ' • ' + host : ''}`;
...
async function searchByTitle(query, year) {
    if (!query) return [];
    const d = await fetchJson(`${BASE_URL}/search.php?q=${encodeURIComponent(query + (year ? ' ' + year : ''))}&page=1&per_page=15`, { headers: mobileHdrs() });
...
async function fetchPostContent(postId, link) {
    ...
    try {
        const r = await fetchSafe(`${BASE_URL}/wp-json/wp/v2/posts/${postId}`, { headers: mobileHdrs() });
        if (r && r.ok) {
...
async function extractSingleVc(vcUrl, referer, targetSeason, targetEp, label, fallbackQ, mediaInfo) {
    ...
    const isHub = lower.includes('hubcloud'), latestBase = isHub ? HC_URL : VC_URL, cur = origin(vcUrl);
